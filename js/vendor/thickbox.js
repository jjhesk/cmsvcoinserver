...
//add thickbox to href & area elements that have a class of .thickbox
    function tb_init(domChunk) {
        $('body').on('click.init_thickbox', domChunk, tb_click);
    }
        ...
        try {
            if (typeof document.body.style.maxHeight === "undefined") {//if IE 6
                ...
                if (document.getElementById("TB_HideSelect") === null) {//iframe to hide select elements in ie6
                    $("body").append("<iframe id='TB_HideSelect'>" + thickboxL10n.noiframes + "</iframe><div id='TB_overlay'></div><div id='TB_window'></div>");
            ...
            if (urlType == '.jpg' || urlType == '.jpeg' || urlType == '.png' || urlType == '.gif' || urlType == '.bmp') {//code to show images
                ...
                imgPreloader.onload = function () {
                    ...
                    if (!(TB_PrevHTML === "")) {
                        function goPrev() {
                            if ($d.unbind("click", goPrev)) {
                                $d.unbind("click", goPrev);
                            }
                            $("#TB_window").remove();
                    ...
                    $d.bind('keydown.thickbox', function (e) {
                        if (e.which == 27) { // close
                            tb_remove();
                ...
                if (url.indexOf('TB_iframe') != -1) {// either iframe or ajax window
                    ...
                    if (params['modal'] != "true") {//iframe no modal
                        ...
                        $("#TB_overlay").unbind();
                        $("#TB_window").append("<iframe frameborder='0' hspace='0' src='" + urlNoQuery[0] + "' id='TB_iframeContent' name='TB_iframeContent" + Math.round(Math.random() * 1000) + "' onload='tb_showIframe()' style='width:" + (ajaxContentW + 29) + "px;height:" + (ajaxContentH + 17) + "px;'>" + thickboxL10n.noiframes + "</iframe>");
                    }
                } else {// not an iframe, ajax
                    if ($("#TB_window").css("visibility") != "visible") {
                        if (params['modal'] != "true") {//ajax no modal
                            $("#TB_window").append("<div id='TB_title'><div id='TB_ajaxWindowTitle'>" + caption + "</div><div id='TB_closeAjaxWindow'><a href='#' id='TB_closeWindowButton'><div class='tb-close-icon'></div></a></div></div><div id='TB_ajaxContent' style='width:" + ajaxContentW + "px;height:" + ajaxContentH + "px'></div>");
                        } else {//ajax modal
                            $("#TB_overlay").unbind();
            ...
            $d.trigger("onShown", [$("#TB_window"), $("#TB_ajaxContent"), $("#TB_closeWindowButton")]);
        } catch (e) {
            //nothing here
        }
    }
