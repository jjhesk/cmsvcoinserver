...
// only load on select pages
            ...
            if (version_compare($wp_version, '3.5', '<')) {
                add_thickbox();
                wp_enqueue_script("mpt-featured-image", LIBJS_ADMIN_MODEL . 'multi-post-thumbnails-admin.js', array('jquery', 'media-upload'));
            } else { // 3.5+ media modal
                wp_enqueue_media();
                wp_enqueue_script("mpt-featured-image", LIBJS_ADMIN_MODEL . 'js/multi-post-thumbnails-admin.js', array('jquery', 'set-post-thumbnail'));
                wp_enqueue_script("mpt-featured-image-modal", LIBJS_ADMIN_MODEL . 'js/media-modal.js', array('jquery', 'media-models'));
            }
        }
         ...
         * @param string $post_id Optional. Post ID.
         * @param int|string $size Optional. Image size. Defaults to 'post-thumbnail', which theme sets using set_post_thumbnail_size( $width, $height, $crop_flag );.
